...
export default {
    globals: {
        __IS_DEV__: true,           // добавляем переменную, т.к. jest про нее должен знать
        __API__: '',
        __PROJECT__: 'jest',
    },
    ...
    modulePaths: [      // добавляем возможность работы с абсолютным импортом
        '<rootDir>src', // https://stackoverflow.com/questions/50863312/jest-gives-cannot-find-module-when-importing-components-with-absolute-paths

    ],
    testMatch: [                     // регулярные выражения, по которым мы находим файлы с тестами
        // '**/__tests__/**/*.[jt]s?(x)',
        // '**/?(*.)+(spec|test).[tj]s?(x)',
    ...
    moduleNameMapper: {
        ...
        '\\.svg': path.resolve(__dirname, 'jestEmptyComponent.tsx'), // 'mock' для всех импортов svg
    },
    reporters: [
        'default',
        ['jest-html-reporters', {
            ...
            // openReport: true,
            inlineSource: true,         // делаем отчет в одном html-файле
        }],
    ],
